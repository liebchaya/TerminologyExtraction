 ...
 *
 */
public class TVScorer implements TermScorer{
	...
	public double score(String term) throws IOException {
		...
		double avgF = sum/D;
		for(Integer docId:spanDocsMap.keySet())
			sigma += Math.pow(spanDocsMap.get(docId)-avgF,2);
		long zeroCount = D-spanDocsMap.size();
		sigma += ((Math.pow(0-avgF,2))*zeroCount);
		return sigma;
